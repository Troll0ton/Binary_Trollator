...
Когда получено представление кода в промежуточном виде, нетрудно перевести его в бинарный требуемой архитектуры. Нужно построить логику работы каждой операции и заменить запись в IR на набор соответствующих инструкций. 
...
Отдельное внимание стоит уделить процессу трансляции операций, работающих с **оперативной памятью**. В начале транслированного кода я помещаю в регистр **R12**указатель на выделенный заранее участок памяти. Значение данного регистра сохраняется на протяжении всей работы программы.
...
И инструкция с обращением к оперативной памяти соответственно:
...
Так как работа моего виртуального процессора основана на обратной польской записи, то было решено не менять концепцию и в конечном бинарном коде:
...
Самым сложным в трансляции данных инструкций являлся правильное изменение адресацииБ логика которой аналогична ранее приведенному процессу в **IR**. 
...
После завершения трансляции бинарного кода менялись права на запись буфера, в котором он лежал. После этого происходило его исполнение. 

---

## Вторая часть - AOT-компилятор <a name="second_part"></a>
